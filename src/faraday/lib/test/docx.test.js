...
import { buildApprovalNoteDocx } from "../deliverables/docx.js";
...
test("produces a well-formed zip archive with the required OOXML parts", () => {
	const parts = unzipToText(Buffer.from(buildApprovalNoteDocx(sampleNote())));
...
test("the footer carries the content hash and names the note as not pre-authored", () => {
	const parts = unzipToText(Buffer.from(buildApprovalNoteDocx(sampleNote({ contentHash: "abc123" }))));
