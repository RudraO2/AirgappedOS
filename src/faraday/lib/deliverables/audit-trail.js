 ...
 * @param {number} [context.images] - attached pictures the model was sent this turn.
...
export function buildAuditTrail(context = {}) {
	...
	// 3. What the model was actually shown.
	//
	// This line read "N OCR lines read from ..." until 31 August 2026. ADR-0008
	// removed the OCR service, so the honest question is no longer how the text
	// was extracted but whether the model saw the picture at all — a note written
	// about an image the model was never sent is the failure this line exists to
	// make visible.
	const images = context.images ?? 0;
	if (images > 0) {
		lines.push(`Source: ${images} attached image${images === 1 ? "" : "s"} sent to the vision model on this machine.`);
	} else {
		lines.push("Source: no image was attached to this turn; the reply came from the conversation alone.");
	}
