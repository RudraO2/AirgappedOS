 ...
 * @returns {Uint8Array} the complete `.docx` file.
 */
export function buildApprovalNoteDocx(note) {
	...
	// fflate writes a standard DEFLATE zip with a CRC per entry — the same
	// container the prototype's hand-written `zip.js` produced over `node:zlib`.
	return zipSync(
		{
			"[Content_Types].xml": utf8(contentTypesXml),
			"_rels/.rels": utf8(rootRelsXml),
			"docProps/core.xml": utf8(coreXml),
			"docProps/app.xml": utf8(appXml),
			"word/document.xml": utf8(documentXml),
			"word/_rels/document.xml.rels": utf8(documentRelsXml),
			"word/footer1.xml": utf8(footerXml),
		},
		{ level: 6, mtime: new Date(note.generatedAt) },
	);
}
